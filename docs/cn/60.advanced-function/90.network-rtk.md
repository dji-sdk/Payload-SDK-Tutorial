## 概述
DJI开放机载的网络RTK能力，结合无人机4G模块联网能力，可以使用网络RTK服务实现厘米级精度定位需求。

## 如何使用机载网络RTK功能

> **说明：** 该功能适用于M400 + 妙算3搭配使用。
1. 开启网络RTK服务


步骤1： 通过`DjiNetworkRtk_RegReceiveNetworkRtkStateCallback`注册网络RTK服务状态接收函数, 以便接收网络RTK服务状态 。

步骤2： 通过`DjiNetworkRtk_StartService`接口开始RTK服务, 检查网络RTK服务状态，当状态为`DJI_REBOOTING`时，需要手动重启飞机，并再次使用该接口进行启动。当为`DJI_TRANSFER`时，网络RTK服务启动成功。
...
> **说明：** 通过该接口注册的回调函数，可以获取RTK服务响应`DjiNetworkRtk_StartService` 接口的运行状态，如果状态是`DJI_REBOOTING`，意味着您需要手动重启一次飞机，机载网络RTK服务自动完成RTK定位源切换，需要注意的是，重启后，仍需要再次调用`DjiNetworkRtk_StartService` 接口开启服务，当状态为`DJI_TRANSFER`时，说明RTK服务启动成功。
...
当不使用机载网络RTK服务时，通过如下接口停止网络RTK服务，RTK服务响应后，可以收到`DJI_STOPPING`状态。
```c
/**
