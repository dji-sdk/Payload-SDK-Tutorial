...
* 时间戳：无人机上的视觉传感器同时共享一个计时器，有关时间同步的详细说明请参见 [时间同步](https://developer.dji.com/doc/payload-sdk-tutorial/cn/basic-function/time-syn.html)。
...
## 使用开放AR能力接口
开放AR能力接口提供对码流的AR图形绘制能力，可以在Pilot码流界面上绘制出点、线、立体多边形、圆柱体。
> **注意：** 该功能需搭载妙算3使用
...
```
### 5. AR图形的绘制通知
在pilot退出liveview界面时，再次进入界面，原有的AR绘制图形会丢失。应用可以通过注册重刷新的通知处理函数，在收到重刷新的通知时，调用AR绘制接口进行重新绘制
```c
...
> * widget_config.json 是一个用于配置控件静态属性的文件，在这里我们新增了AR 图形绘制的样式配置，开发者可以根据需要更改配置的属性值。
...
## 使用PSDK编解码实现AI识别功能
> **说明**
> * 当前仅在M400无人机挂载H30相机和DJI 妙算3场景下，支持妙算3直接获取H30相机码流解码后的图像，并支持使用PSDK提供的硬件编码接口将处理后的图像编码为H.264码流。
> * 编解码回调函数中均不允许进行阻塞耗时操作，推荐创立独立线程进行目标识别等功能。

1. 注册目标识别结果类别信息
调用`DjiLiveview_RegUserAiTargetLableList`接口，可向DJI PILOT注册用户自定义的目标类别信息，用于作AI识别结果渲染。
...
2. 获取解码后的图像数据
DJI PSDK支持用户获取相机码流解码后的NV12\RGB PLANAR\RGB PACKED格式的图像数据，例如：调用以下接口：`DjiLiveview_StartImageStream(CameraPostion, MediaResource, PIXFMT_RGB_PACKED ,DjiLiveview_RcvImageCallback);`，可以订阅到挂载位置为`CameraPostion`, 码流ID为`MediaResource`的RGB PACKED格式图像数据，并向其注册回调函数`DjiLiveview_RcvImageCallback`用以接收解码后的图像数据。
...
3. 初始化硬件编码器
初始化内置编码器，例如：调用以下接口：`DjiLiveview_RegEncoderCallback(DjiLiveview_EncoderUseCallback)`可以初始化硬件解码器，并向其注册回调函数`DjiLiveview_EncoderUseCallback`用以接收编码器输出的H.264码流数据;
...
4. AI识别结果编码
用户可以将AI识别的目标信息填写为`T_DjiLiveViewStandardMetaData`格式，编码到码流中，该格式的码流可以回传到DJI PILOT上进行目标识别结果渲染在设备图传页面。 例如使用接口`DjiLiveview_EncodeAFrameToH264(buf, len,imageInfo, metaData)`，可以将图像原始信息结合识别结果编码为H.264格式码流数据，再使用`DjiPayloadCamera_SendVideoStream`可将码流回传至Pilot进行识别结果显示。
...
5. AI识别结果回传到APP
用户可使用`DjiLiveview_SendAiMetaToPilot(metaData)`直接将`T_DjiLiveViewStandardMetaData`格式的AI识别结果回传到PILOT，该结果会直接渲染在H30相机图传页面。

```c
returnCode = DjiLiveview_SendAiMetaToPilot(metaData);
```

妙算3支持获取挂载在 M400 飞机上的 H30T 相机实时码流（1080P/4K）解码后的图像数据，并利用自身 AI 算力进行目标识别。识别结果可通过硬件编码接口融入码流，或单独推送至 Pilot 界面进行渲染显示。
...
> **说明：** Pilot 图传页面渲染出妙算3 AI识别码流框，需要以dpk应用形式完成安装, 妙算3应用开发请参考 "妙算3应用开发指南" 章节。
