...
:::warning 注意：
- 因为 FTS 触发是飞行周期一次性行为，所以只提供回调函数的注册接口，不提供注销接口。
- FTS 功能的使用有机型差异。
  - 对于 Matrice 400机型，负载设备需要将频率为400Hz，占空比为50%的 PWM 波形输出到第4个 E-Port V2接口的 FTS 引脚。
  - 对于 Matrice 3D 机型，请按后续说明使用 FTS 功能。
:::
### 1. 初始化飞行控制模块
...
```
## EFTS 功能
EFTS 通过负载接口向飞行平台输出特定规格的硬件 PWM 信号，在满足电压、频率和占空比等要求时，飞行平台会立即执行 **电机停桨/停转** 操作，而不依赖于软件指令链路。
::: warning 注意：
Matrice 4D/Matrice 4TD、Matrice 4E/Matrice 4T 及 Matrice 400 均支持 EFTS 功能。  
其中，Matrice 4D/Matrice 4TD 支持 E‑Port Lite 接口；Matrice 4E/Matrice 4T 支持 E‑Port 和 E‑Port Lite 接口；Matrice 400 仅支持 4 号接口。  
负载接口 FTS 的硬件信号要求如下：电压为 3.3 V，占空比为 50%，脉冲频率为 200 Hz。
:::
## 使用 EFTS 功能

1. 在地面端使能 EFTS 功能触发源，选择/启用飞控侧的 FTS（电机停止）PWM 触发端口。
```c
T_DjiReturnCode DjiFts_SelectFtsPwmTrigger(E_DjiMountPosition position);
```

2. 负载通过输出 FTS 硬件信号触发停桨。可以通过订阅 DJI_FC_SUBSCRIPTION_TOPIC_ESC_DATA 信号判断停桨结果。