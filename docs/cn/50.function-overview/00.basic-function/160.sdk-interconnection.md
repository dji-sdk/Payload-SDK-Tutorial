...
* 使用对象：4 个 E-Port 接口（PSDK）；若 E4 接口外接扩展坞，则最多支持 7 个 E-Port 接口（PSDK）；遥控器 App（MSDK）。
* 使用范围：任意 E-Port 接口上的 PSDK 均可与 MSDK 建立通信，任意两个 E-Port 接口上的 PSDK 之间也可建立通信。
* 使用约束：
  * 暂不支持双控的场景。
  * 暂不支持多个 E-Port 口的 PSDK 同时与 MSDK 或其他 E-Port 口的 PSDK 建立通信。
