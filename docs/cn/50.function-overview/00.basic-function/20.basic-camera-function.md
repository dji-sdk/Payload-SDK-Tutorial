...
:::warning 注意：
 相机只能在一种模式中执行相应的操作，如在录像模式下**仅能录像无法拍照**。
:::
...
:::tip 说明：
支持动态预览的文件格式：MP4、JPG、DNG 和 MOV，编码格式请参见<a href="../../payload-quick-start/configure-the-hardware-environment/payload-develop-criterion.html#视频流传输标准" target="_blank" rel="noopener noreferrer">视频标准</a>。
:::
...
:::tip 说明：
 相机开始录像后，DJI Pilot 及基于 MSDK 开发的移动端 App 会显示当前正在录像的时间，相机停止录像时，该时间将归 0。
:::
...
:::tip 说明：
 若按钮为黄色，表示当前为测光功能，点击后，即可切换为对焦模式。
:::
...
:::tip 说明：
  在 DJI Pilot 上使用相机类负载设备的测光功能时，若按钮为绿色，表示当前为对焦功能，点击后，即可切换为测光模式。 
:::
...
:::tip 说明：
根据实际的使用需要，可设置相机类负载设备默认的变焦倍数，当前为 1.0。
:::
<div>
