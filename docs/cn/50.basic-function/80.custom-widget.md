...
<div>
    ...
    <div align="center">
        <img src="https://terra-1-g.djicdn.com/71a7d383e71a4fb8887a310eb746b47f/psdk/V3.3/custom-widget-main-screen.png" style="max-width: 80%; height: auto;">
    </div>
</div>
...
## 控件管理
在搭载多个PSDK负载设备的M400无人机上，支持在Linux系统上开发的PSDK设备使用控件管理功能，下载其他PSDK设备的自定义控件文件，并订阅和改变其控件值。
...
* 下载挂载在`position`的位置的PSDK设备所注册的自定义控件文件，并注册回调函数`DjiWidgetManager_UsrDownloadCallback`，用以接收下载得到的数据。
```c
DjiWidgetManager_RegDownloadFileDataCallback(position, DjiWidgetManager_UsrDownloadCallback);
```

* 订阅挂载在`position`的位置的PSDK设备所注册的自定义控件状态，并注册回调函数'DjiTestWidgetManager_RecvWidgetStatesCallback'，当控件状态发生改变时，该回调函数会收到通知。
```c
DjiWidgetManager_SubscribePayloadWidgetStates(position, DjiTestWidgetManager_RecvWidgetStatesCallback);
```

* 设置挂载在`position`的位置的PSDK设备状态为`state`。
```c
DjiWidgetManager_SetWidgetState(position, state);
```

* 取消订阅挂载在`position`的位置的PSDK设备所注册的自定义控件状态，取消后将不再收到其控件状态改变的通知。
```c
DjiWidgetManager_UnsubscribePayloadWidgetStates(position);
```

* 在M400无人机上，DJI PSDK提供了通过设置控件值控制DJI V1喊话器的接口。
* 订阅挂载在`position`的位置的DJI V1喊话器控件状态，并注册回调函数`DjiTestWidgetManager_RecvSpeakerStatesCallback`，当喊话器控件状态发生改变时，该回调函数会收到通知。
```c
DjiWidgetManager_SubscribeSpeakerStates(position, DjiTestWidgetManager_RecvSpeakerStatesCallback);
```

* 向挂载在`position`的位置的DJI V1喊话器发送音频文件。
```c
DjiWidgetManager_SendSpeakerAudioData(position, &audioFileInfo);
```

* 设置挂载在`position`的位置的DJI V1喊话器参数（音量、播放模式、播放内容）。
```c
DjiWidgetManager_SetSpeakertState(position, &param);
```

* 取消订阅挂载在`position`的位置的的DJI V1喊话器控件状态。
```c
...
* [Payload SDK 数据传输功能](https://developer.dji.com/doc/payload-sdk-tutorial/cn/basic-function/data-transmission.html)
