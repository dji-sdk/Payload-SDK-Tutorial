...
接入 M400 无人机 EPROT 口的 DJI 妙算3支持使用`DjiPowerManagement_OutputHighPower(true)`接口,通过 usb 口向负载输出 14V 高电压。
...
## 系留模式支持
M400 无人机支持系留模式，当无人机处于系留模式下时，开发者可以创建系留电缆长度推送线程，定频推送线长，建议频率10Hz,线长范围0-300m。当剩余线缆长度为15m和5m时，PILOT会进行安全飞行提示，并对飞行器进行限速。需注意该接口仅适用于基于 E-Port V2 同轴线开发的负载设备。
