...
##### 2. 格式转换
使用PSDK 开发的相机类负载设备通过`DjiPlayback_VideoFileTranscode`接口，将用户指定的媒体文件转换为 H.264 格式的文件，用户使用DJI Pilot 以及基于MSDK 开发的移动端App 能够向负载设备发送转换媒体文件格式的指令，将指定的文件转为H.264 格式，同时获取转换后媒体文件的帧率、帧信息、总帧数等信息。
...
> **说明：** 有关实现视频流传输功能的详细说明和具体步骤请参见[视频流传输](https://developer.dji.com/doc/payload-sdk-tutorial/cn/basic-function/camera-video-stream-transmission.html)。
