使用X-Port、SkyPort V2或者SDK 同轴线开发负载设备时，请按本文引导连接硬件平台、负载设备开发板、接口拓展板和DJI 的无人机。
...
> **说明：**
> Matrice 300 RTK、Matrice 30系列与 Mavic 3 行业系列在供电接口、USB_VBUS和USB_ID接口上存在差异，在软硬件设计上需要考虑兼容性。
...
* 供电接口：
  ...
  * Mavic 3 行业系列默认做USB主设备，USB_ID引脚（A6）必须拉低。

  * 飞行器无电压输出能力，只能检测外部电压的有无，USB_VBUS引脚（A7）接收外部电源输入。
