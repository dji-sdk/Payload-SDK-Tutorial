...
E-Port 开发者套件用于将飞行器的 E-Port 接口转换成多种标准硬件接口，方便开发者进行硬件设备连接和 SDK 开发调试。E-Port 开发者套件支持的飞行器机型与适配的机身接口，请阅读[表. 标准硬件接口与开发套件](https://developer.dji.com/doc/payload-sdk-tutorial/cn/payload-quick-start/drone-port.html)。
...
2. 完成[硬件环境准备](#1-准备硬件设备)并启动完成后，我们首先输入您配置的账号密码，检查是否可以登录进入 Raspberry Pi 桌面系统。
...
### 2. 开启树莓派 4B 的USB设备功能
**情形一：使用EPort接口开发SDK设备**

您可以参考[树莓派 bulk 端点配置指导](https://sdk-forum.dji.net/hc/zh-cn/articles/10232604141465-M30%E5%BC%80%E5%8F%91-%E6%A0%91%E8%8E%93%E6%B4%BE4B%E9%85%8D%E7%BD%AEUSB-device-RNDIS-%E5%92%8C-BULK)开启树莓派 4B 的USB设备功能。

**情景二：使用E-Port V2开发SDK设备**
> 此设置仅在 Matrice 400 机型上需要配置。

您可以参考[树莓派 bulk 端点配置指导](https://sdk-forum.dji.net/hc/zh-cn/articles/10232604141465-M30%E5%BC%80%E5%8F%91-%E6%A0%91%E8%8E%93%E6%B4%BE4B%E9%85%8D%E7%BD%AEUSB-device-RNDIS-%E5%92%8C-BULK)开启树莓派 4B 的 USB设备功能。

需要注意的是，您需要将`raspi-usb-device-start.sh`替换为以下内容。
当使用E-Port V2开发SDK设备时，树莓派同一时间仅能作为VCOM或RNDIS或USBBULK设备使用。

> 当使用BULK功能时，需要将VID:PID设置为2CA3:F001。<br>
> 当使用VCOM功能时，需要将VID:PID设置为2CA3:F002。<br>
> 当使RNDIS功能时，需要将VID:PID设置为2CA3:F003。<br>
...
如果您使用的连接方式是DJI_USE_ONLY_UART，您需要在以下配置文件中对当前使用的USB串口设备名称和VID、PID进行配置。
...
如果您使用的连接方式是DJI_USE_UART_AND_NETWORK_DEVICE或DJI_USE_ONLY_NETWORK_DEVICE，您需要在以下配置文件中对当前使用的网卡设备名称和VID、PID进行配置。
...
如果您使用的连接方式是DJI_USE_UART_AND_USB_BULK_DEVICE或DJI_USE_ONLY_USB_BULK_DEVICE，您需要在以下配置文件中对当前使用的USB bulk端点进行配置。
...
PSDK 支持[自定义控件功能](https://developer.dji.com/doc/payload-sdk-tutorial/cn/basic-function/custom-widget.html)，如下 Sample 将演示如何使用 API 接口，Pilot 上会显示预设的自定义控件 UI。
...
PSDK 支持 [信息管理功能](https://developer.dji.com/doc/payload-sdk-tutorial/cn/basic-function/info-management.html)和[飞行控制功能](https://developer.dji.com/doc/payload-sdk-tutorial/cn/advanced-function/flight-control.html)，如下 Sample 将演示如何使用 API 接口，订阅到 M350 RTK 飞行器遥感数据和控制飞行器运动。
...
PSDK 支持[相机管理功能](https://developer.dji.com/doc/payload-sdk-tutorial/cn/advanced-function/basic-camera-management.html)和[云台管理功能](https://developer.dji.com/doc/payload-sdk-tutorial/cn/advanced-function/gimbal-management.html)，如下 Sample 将演示如何使用 API 接口，控制相机拍照和云台旋转。不同的相机机型对相机管理功能的支持情况请查阅[相机管理](https://developer.dji.com/doc/payload-sdk-tutorial/cn/advanced-function/basic-camera-management.html)中的**相机支持功能差异概览表格**。
...
PSDK 支持[获取相机码流功能](https://developer.dji.com/doc/payload-sdk-tutorial/cn/advanced-function/liveview.html)，如下 Sample 将演示如何使用 API 接口，订阅到 M350 RTK FPV 和主相机的码流。
