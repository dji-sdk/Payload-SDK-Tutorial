...
::: tip For more help, please refer to the following resources:
- Visit the [PSDK Knowledge Base](https://sdk-forum.dji.net/hc/en-us/categories/360001790873-Payload-SDK).
- Submit a [support request](https://sdk-forum.dji.net/hc/en-us/requests/new) to obtain technical support.
- Send an <a href="mailto:dev@dji.com" target="_blank">email</a> to the DJI SDK team.
:::


## How should the hardware be connected for each aircraft model?
For details, refer to [PSDK connection of various aircraft models](https://sdk-forum.dji.net/hc/en-us/articles/34834641217305-PSDK-connection-of-various-aircraft-models).
...
## How to Configure Auto Startup Using `systemd --user`?

If your application does not need to be packaged and distributed as a DPK, or if it depends on a complex runtime environment such as ROS and is not suitable for DPK packaging, you can use the following method to configure auto startup with `systemd --user`.

#### 1. Create a test script `hello.sh`
...
#### 2. Run the script manually and verify that logs are written correctly
...
#### 3. Create the user unit directory
...
**Notes:**
- `%h` is automatically expanded to the current user's home directory, so there is no need to hard-code the username path.
- `Type=oneshot` is suitable for scripts that run once and then exit.

#### 5. Start the service and configure auto startup without root privileges

- **Reload the user-level systemd configuration:**
...
- **Verify whether the service has been enabled:**
