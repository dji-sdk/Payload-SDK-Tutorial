## Stardard Hardware Port Introduction
...
* Power Output:
    * If you need to power the payload equipment, please use the VCC pin.
  ...
  * Only when the USB_VBUS pin (A7) is pulled down, then 5V voltage is ouotput. Maximum current is 500mA. The USB_VBUS pin (A7) of Mavic 3E/3T have no voltage output ability. It can only detect the presence or absence of external voltage. 
...
The gimbal port supports using with [DJI Official Gimbal Connector](https://store.dji.com/list/education-and-industry?from=store-nav&tab=104&sort=recommendation&page=1&category=101%3D101.210&set_region=US), the pin definition is not opened. But you can develope through SkyPort V2 or X-Port. 
<div>
<div align=center>
