:::tip Tips
Before running the "Precise Positioning" sample code, please use the APP developed by DJI Pilot or MSDK to check the good communication status between the drone and RTK satellites to ensure that the payload device can obtain accurate positioning results. Figure 1. RTK signal states as shown.
:::
...
<img src="https://terra-1-g.djicdn.com/84f990b0bbd145e6a3930de0c55d3b2b/admin/doc/93069b77-81e4-419d-ae39-6fba08a41737.png" width="50%" alt/></span></p>
</div></div>
...
* Target point: The position where the positioning information is actually obtained, such as the center point of the gimbal port.
  ...
  :::tip Note: 
  - On Matrice 300 RTK, the target point is the center point of the upper surface of the load device adapter ring, and the load device needs to be mounted on the drone No. 1 gimbal.
  - On Matrice 400, the target point is the center point of the upper surface of the payload adapter ring, and access via SkyPort V2/SkyPort V3 is supported on ports E1 and E2.
  ::: 
...
:::tip Note:
 When obtaining precise positioning, you need to use the time synchronization function to synchronize the local time of the load device to the drone time. For details on using the time synchronization function, please refer to <a href="../basic-function/time-syn.html" target="_blank" rel="noopener noreferrer">Time Synchronization</a>.
:::
...
:::tip Note:
 The time of the drone (the time of the drone system) when the positioning event occurs should be earlier than the rising edge time of the latest PPS signal, and the time interval must be within 1~2s, as shown in Figure 2. Precise positioning shown.
:::
