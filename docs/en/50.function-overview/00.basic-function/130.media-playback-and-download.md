...
:::tip Note:
 If you use a virtual machine to debug camera-like payload devices, please set the mode of the virtual machine's network adapter to bridge mode, and enable "Copy physical network connection state".
:::

### Register the function in playback download
:::tip Note:
 Users can register the video playback and download function according to the actual use needs.
:::
...
:::tip Note:
 The default path of media files is `camera_media_emu/media_file`, users can change the path of media files according to actual needs.
:::
...
:::tip Note:
 After DJI Pilot and the Mobile App developed based on MSDK receive the thumbnail image on the camera-like payload device, it will be cached locally in the APP.
:::
...
:::tip Note:
 The download speed of media files is about 300KB/s, and the actual download speed may vary due to the working environment of the aircraft.
:::
