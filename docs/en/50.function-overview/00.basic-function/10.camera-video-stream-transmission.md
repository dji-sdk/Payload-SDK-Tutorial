## Overview
Using the video stream transmission control function provided by PSDK, the developer needs to realize the function of obtaining the code stream of the video stream file first, according to the H.264 encoding format encode the video stream, and call the specified interface to send the video stream data in combination with the video frame rate and other parameters. Users can obtain the real-time video data on the camera class payload device by using DJI Pilot and the Mobile App developed based on MSDK.

:::tip Related References
* Please refer to <a href="../../payload-quick-start/configure-the-hardware-environment/payload-develop-criterion.html#video-stream-transmission-standard" target="_blank" rel="noopener noreferrer">Video Standard</a> for the detailed description of the video stream format.
...
:::tip Note:
 For Linux systems with the `network-manager` service, you need to delete all the configuration information of the `network-manager` service, otherwise the network parameters cannot be successfully configured.
:::
...
:::tip Note:
 The video streaming function of PSDK only supports the transmission of video streaming files in H.264 format. For details of the H.264 format standard, please refer to <a href="../../payload-quick-start/configure-the-hardware-environment/payload-develop-criterion.html#video-stream-transmission-standard" target="_blank" rel="noopener noreferrer">Video Standard</a>.
:::
...
:::tip Note:
 The longest frame that can be sent by a camera payload device developed based on PSDK is 65K. If the length of the frame exceeds 65K, the frame needs to be disassembled and sent.
:::
