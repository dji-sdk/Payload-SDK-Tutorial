...
<div align=center><img src="https://terra-1-g.djicdn.com/71a7d383e71a4fb8887a310eb746b47f/psdk/V3.3/custom-widget-main-screen.png" style="max-width: 80%; height: auto;"></div>
</div>
...
<div align=center><img src="https://terra-1-g.djicdn.com/71a7d383e71a4fb8887a310eb746b47f/psdk/V3.3/text-input-screen.png" style="max-width: 80%; height: auto;"></div>
</div>
...
:::tip Note:
 The control configuration file includes <b>static configuration file</b> and <b>control UI icon</b>, it is recommended to configure control properties in <b>static configuration file</b> first , and then design the control icon.
:::
...
{
    "version": {
        // The version of the custom control configuration file, the user cannot change this version number
        "major": 1, "minor": 0
    }
    ,
    "main_interface": {
        // DJI Pilot main interface control settings
        "floating_window": {
        ...
        "widget_list": [ // Main interface action bar control list
            ...
            "icon_file_set": {
                // control icon file set
                name of the control icon file in the selected state "icon_file_name_unselected": "icon_button1.png" // The name of the control icon file in the unselected state
            }
        }
        ,
            {
            ...
            "icon_file_set": {
                "icon_file_name_selected": "icon_button2.png", "icon_file_name_unselected": "icon_button2.png"
            }
        }
        ,
            {
            ...
            "list_item": [ {
                "item_name": "Item_1",
                "icon_file_set": {
                    "icon_file_name_selected": "icon_list_item1.png", "icon_file_name_unselected": "icon_list_item1.png"
                }
            }
            ,
                {
                "item_name": "Item_2",
                "icon_file_set": {
                    "icon_file_name_selected": "icon_list_item2.png", "icon_file_name_unselected": "icon_list_item2.png"
                }
            }
            ]
        }
        ,
            ...
            "widget_name": "Switch",
            "icon_file_set": {
                "icon_file_name_selected": "icon_switch_select.png", "icon_file_name_unselected": "icon_switch_unselect.png"
            }
        }
        ,
            {
            ...
            "widget_name": "Scale",
            "icon_file_set": {
                "icon_file_name_selected": "icon_scale.png", "icon_file_name_unselected": "icon_scale.png"
            }
        }
        ]
    }
    ...
    "config_interface": {
        ...
        ,
        "widget_list": [ {
            "widget_index": 5, "widget_type": "button", "widget_name": "Button 5"
        }
        ,
            {
            "widget_index": 6, "widget_type": "scale", "widget_name": "Scale 6"
        }
        ,
            {
            "widget_index": 7, "widget_type": "int_input_box", "widget_name": "Integer Input Box 7", "int_input_box_hint": "unit:s"
        }
        ,
            {
            "widget_index": 8, "widget_type": "switch", "widget_name": "Switch 8"
        }
        ,
            {
            "widget_index": 9,
...
:::tip Notes: 
The configuration items in the control configuration files in different system languages, such as control number, control quantity and control type , need to be consistent.
:::
...
<div style="width:100%; display: flex; flex-direction: column; align-items: center;">
    ...
    <table id="t02" style="border-collapse: collapse; min-width: 350px;">
        <thead>
            <tr>
                <th style="text-align: left;">Control Type</th>
                <th style="text-align: left;">Reserved Keys</th>
        ...
        <tbody>
            <tr>
                ...
                <td style="text-align: left;" rowspan="3">Keys: C1–C4 / BA–BH</td>
                <td style="text-align: left;">
                    &bull; Key Press: Triggers the "Button" control<br> &bull; Key Release: Triggers the "Button" control release
                </td>
            </tr>
            <tr>
                <td style="text-align: left;">Switch</td>
                <td style="text-align: left;">
                    Alternate Trigger: Press and release the key to toggle the "Switch" control on or off
                </td>
            </tr>
            <tr>
                <td style="text-align: left;">Dropdown List</td>
                <td style="text-align: left;">
                    Click Trigger: Press and release the key to select the next item
                </td>
            </tr>
            <tr>
                <td style="text-align: left;">Range Bar</td>
                <td style="text-align: left;">Left/Right Levers: LS / RS</td>
                <td style="text-align: left;">
                    Toggle Trigger: Move the lever to adjust the range bar value
                </td>
            </tr>
            <tr>
                <td style="text-align: left;">Input Box</td>
                <td style="text-align: left;">Focus Knob: TD</td>
                <td style="text-align: left;">
                    Knob Trigger: Turn the focus knob to modify the input box value
                </td>
            </tr>
        </tbody>
    </table>
...
<div align=center><img src="https://terra-1-g.djicdn.com/71a7d383e71a4fb8887a310eb746b47f/psdk/V3.3/custom-widget-action.png" style="max-width: 80%; height: auto;"></div>
</div>
...
<div align=center><img src="https://terra-1-g.djicdn.com/84f990b0bbd145e6a3930de0c55d3b2b/admin/doc/f9e3e2cd-3886-4e63-8d86-0c60690a6482.png" style="max-width: 80%; height: auto;"></div>
</div>
...
## Payload Management
### Widget Management
On the Matrice 400 aircraft equipped with multiple PSDK devices, the widget management function supports PSDK devices developed on the Linux system. It allows downloading custom widget files of other PSDK devices and subscribing to and changing their values.


* Initialize the widget management module.
```c
DjiWidgetManager_Init();
```

* Download the custom widget file registered by the PSDK device mounted at the `position` and register the callback function `DjiWidgetManager_UsrDownloadCallback` to receive the downloaded data.
```c
DjiWidgetManager_RegDownloadFileDataCallback(position, DjiWidgetManager_UsrDownloadCallback);
```

* Subscribe to the custom widget status registered by the PSDK device mounted at the `position` and register the callback function 'DjiTestWidgetManager_RecvWidgetStatesCallback'. This callback function will be notified when the widget status changes.
```c
DjiWidgetManager_SubscribePayloadWidgetStates(position, DjiTestWidgetManager_RecvWidgetStatesCallback);
```

* Set the status of the PSDK device mounted at `position` to `state`.
```c
DjiWidgetManager_SetWidgetState(position, state);
```

* Unsubscribe from the custom widget status registered by the PSDK device mounted at `position`. After unsubscribing, you will no longer receive notifications of its widget status changes.
```c
DjiWidgetManager_UnsubscribePayloadWidgetStates(position);
```
#### Example
By using the custom widget management feature, payload devices that support widgets can be controlled, achieving the same control experience as operating widgets in DJI Pilot.  
The following example uses the Zenmuse S1 drone spotlight to demonstrate how to control the spotlight through this feature.

**1. Obtain the widget file list**  
Call the `DjiWidgetManager_WidgetDownloadFileList` API to download the list of widget files supported by the payload device. This list includes the widget description files (JSON) and the corresponding icon resource information.

**2. Download the widget description file**  
Based on the file list information, use the `DjiWidgetManager_DownloadFileByIndex` API to download the required widget files. After the download is complete, the widget description file `widget_config.json` can be obtained.

The `widget_config.json` file is used to describe the widget information of the spotlight, including the widget type, widget name, widget icon configuration, and other related settings. This file defines how the spotlight is displayed and how users interact with it on the Pilot side.

The `widget_config.json` file of the Zenmuse S1 drone spotlight is shown below:
```json
{
  "version": {
    "major": 1,
  ...
  "main_interface": {
    ...
    "widget_list": [
      {
        "comment": "Flash mode switch. A switch widget where 0 disables flash mode and 1 enables flash mode.",
        "widget_index": 0,
        "widget_type": "switch",
        "widget_name": "Flash Mode",
        "icon_file_set": {
          "icon_file_name_selected": "icon_blink_select.png",
          "icon_file_name_unselected": "icon_blink_unselect.png"
        },
      ...
      {
        "comment": "Brightness adjustment slider. Values range from 0 to 100, representing the light brightness.",
        "widget_index": 1,
        "widget_type": "scale",
        "widget_name": "Light Brightness",
        "icon_file_set": {
          "icon_file_name_selected": "icon_brightness_select.png",
          "icon_file_name_unselected": "icon_brightness_unselect.png"
        },
        "customize_rc_buttons_config": {
          "is_enable": true,
      ...
      {
        "comment": "Lighting mode selection. 0: Low Beam (flood light), 1: High Beam (spot light), 2: Both On.",
        "widget_index": 2,
        "widget_type": "list",
        "widget_name": "Lighting Type",
        "list_item": [
          {
            "item_name": "Low Beam",
            "icon_file_set": {
              "icon_file_name_selected": "icon_lowbeam_select_inlist.png",
              "icon_file_name_unselected": "icon_lowbeam_unselect_inlist.png"
            }
          },
          {
            "item_name": "High Beam",
            "icon_file_set": {
              "icon_file_name_selected": "icon_highbeam_select_inlist.png",
              "icon_file_name_unselected": "icon_highbeam_unselect_inlist.png"
            }
          },
          {
            "item_name": "Both On",
            "icon_file_set": {
              "icon_file_name_selected": "icon_fullbeam_select_inlist.png",
              "icon_file_name_unselected": "icon_fullbeam_unselect_inlist.png"
            }
          }
        ],
        "customize_rc_buttons_config": {
      ...
      {
        "comment": "Light power switch. 0 turns the light off, 1 turns the light on.",
        "widget_index": 3,
        "widget_type": "switch",
        "widget_name": "Light Switch",
        "icon_file_set": {
...
```
**3. Control the Searchlight by Setting Widget States**  

Based on the widget descriptions defined in `widget_config.json`, the operating state of the searchlight can be changed by setting widget states.  
For example, the following sample code turns on the Zenmuse S1 searchlight mounted on E‑Port V2 port 1:

 ```c
   T_DjiWidgetStates state;
   state.widget_index = 3;
   state.widget_value = 1;
   state.widget_type = DJI_WIDGET_TYPE_SWITCH;

   DjiWidgetManager_SetWidgetState(1, &state);
```
By using this approach, payload devices can retrieve widget information from other payloads and set widget states accordingly, achieving the same control capabilities as operating widget icons in DJI Pilot.  
This mechanism can be used to enable payload linkage control, cross-payload operations, or automated application scenarios.

### Loudspeaker Management
On the Matrice 400 aircraft, DJI PSDK provides an interface to control the DJI V1 speaker by setting widget values.
* Subscribe to the widget status of the DJI V1 speaker mounted at `position` and register the callback function `DjiTestWidgetManager_RecvSpeakerStatesCallback`. This callback function will receive notifications when the speaker status changes.
```c
DjiWidgetManager_SubscribeSpeakerStates(position, DjiTestWidgetManager_RecvSpeakerStatesCallback);
```

* Send audio files to the DJI V1 speaker mounted at `position`.
```c
DjiWidgetManager_SendSpeakerAudioData(position, &audioFileInfo);
```

* Set the parameters (volume, playback mode, playback content) for the DJI V1 speaker mounted at `position`.
```c
DjiWidgetManager_SetSpeakertState(position, &param);
```

* Unsubscribe from the status of the DJI V1 speaker mounted at `position`.
```c
DjiWidgetManager_UnsubscribeSpeakerStates(position);
```

## Related References
* [Mobile SDK](https://developer.dji.com/doc/mobile-sdk-tutorial/en/)
* <a href="./data-transmission.html" target="_blank" rel="noopener noreferrer">Payload SDK data transfer function</a>
