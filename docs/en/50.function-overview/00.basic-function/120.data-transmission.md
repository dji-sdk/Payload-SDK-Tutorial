...
:::tip Note:
- he PSDK data transmission module transfers data transparently between the payload device, onboard computer, and the aircraft. It allows users to design custom data transmission protocols based on actual application requirements, enabling payload devices developed with PSDK to achieve complex communication with mobile apps or onboard computers.  
- Compatibility note: If the video stream format is set to **DJI H264**, the data stream transmission function will be unavailable. It is recommended to switch to the <a href="./sdk-interconnection.html" target="_blank" rel="noopener noreferrer">SDK Interconnection</a> feature instead.
:::
## Basic Concepts
...
:::tip Note:
 The dynamic bandwidth limit of the command signal transmission channel is equal to the static bandwidth limit.
:::
...
:::tip Note
This interface supports small-data transparent transmission between payloads.
:::
...
:::tip Note:
 Only the load device developed based on Linux can obtain the status information of the high-speed data transmission channel.
:::
...
:::tip Note:
 The real-time bandwidth statistics period is 1s.
:::
