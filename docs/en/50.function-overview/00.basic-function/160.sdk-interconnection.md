...
### Matrice 400
* Applicable Components: Four E‑Port ports (PSDK). If a hub is connected to the E4 port, up to seven E‑Port ports (PSDK) are supported. Remote Controller App (MSDK).
* Communication Scope: A PSDK connected to any E‑Port can establish communication with the MSDK, and communication can also be established between PSDK devices connected to any two E‑Port ports.
* Limitations:
  * Dual-operator (dual RC) scenarios are not supported.
  * Multiple E-Port PSDKs are not supported to simultaneously establish communication with the MSDK or with other E-Port PSDKs.
