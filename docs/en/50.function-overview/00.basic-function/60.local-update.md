...
Please refer to <a href="../../payload-quick-start/quick-demo/raspberry-pi.html" target="_blank" rel="noopener noreferrer">Quick Demo</a> when using the "local upgrade" function, and compile the program that currently supports the local upgrade function.
...
:::tip Note:
 ttyUSB0 is the serial port device connected to the load device, the developer can modify the name according to the actual situation
:::
...
To use the local upgrade function on the STM32F4 platform, the developer needs to <a href="../../faq.html#how-to-flash-the-bootloader-on-the-stm32f4-discovery-boardl" target="_blank" rel="noopener noreferrer">burn the Bootloader into the development platform</a>, and then burn the firmware of the load device. The `STM32F4Platform` in the following directory can be `psdk_development_board_1.0` (use the load device development board) or `stm32f4_eval` (use the STM32 development board).
...
:::tip Note:
 When modifying the version number, the version number must not be lower than the current version, that is, when the current firmware version number used by the load device is V02.01.00.03, please change the version number to V02.01.00.04, as follows example shown.
:::
...
:::warning Note:
 The version number in the firmware file name must be consistent with the version number defined in the code, otherwise the load device may fail to upgrade.
:::
...
The load device developed by STM32F4 directly reads the firmware file from the Flash and runs the load device program, so the developer needs to burn the Bootloader first and then the application firmware. For the steps of burning the Bootloader, please refer to <a href="../../faq.html#how-to-flash-the-bootloader-on-the-stm32f4-discovery-boardl" target="_blank" rel="noopener noreferrer">FAQ</a>.
...
:::
:::tip Note:
 `stm32f4_discovery` in the above directory can be either `psdk_development_board_1.0` or `stm32f4_eval`.
:::
