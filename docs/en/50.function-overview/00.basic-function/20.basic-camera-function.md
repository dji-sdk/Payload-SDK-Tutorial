...
:::tip Note:
 The camera can only perform corresponding operations in one mode, such as in video mode **only video recording but not taking pictures**.
:::
...
### Focus point
:::tip Note:
 The focus point of the camera-like payload is the center of the sensor by default.
:::
...
:::tip Note:
 Supported file formats for dynamic preview: MP4, JPG, DNG and MOV, please refer to <a href="../../payload-quick-start/configure-the-hardware-environment/payload-develop-criterion.html#video-stream-transmission-standard" target="_blank" rel="noopener noreferrer">Video Standard</a>.
:::
...
:::tip Note:
 After the camera starts recording, DJI Pilot and the Mobile App developed based on MSDK will display the current recording time. When the camera stops recording, the time will return to 0.
:::
...
:::tip Note:
 If the button is yellow, it means that the metering function is currently used. After clicking, it can be switched to focus mode.
:::
...
:::tip Note:
 When using the metering function of a camera-like payload on the DJI Pilot, if the button is green, it means that the focus function is currently active. After clicking, it can be switched to the metering mode.
:::
...
:::tip Note:
 According to the actual needs, you can set the default zoom factor of the camera-like load device, which is currently 1.0.
:::
<div>
