...
:::tip Note:
 Only after the developer has set all the basic parameters of the waypoint information, can the optional parameters of the waypoint information be set.
:::
...
:::tip Note:
 Waypoint 2.0 function currently only supports Matrice 300 RTK and Matrice 350 RTK models
:::
...
:::tip Note:
 Waypoint 3.0 function currently only supports Matrice 30 Series, Mavic 3 Enterprise Series, and Matrice 3D/3TD models. Do not support Matrice 300 RTK and Matrice 350 RTK.
:::
...
```
## Using the Point of Interest (POI) Mission

:::tip Note:
The POI feature is only supported on the Mavic 3 Enterprise series. Other aircraft models do not currently support this feature.
:::

The POI mission provides capabilities to start and stop point-of-interest circling, set the circling speed, and monitor the mission status.

### 1. Initialize the POI Mission

Before using the POI circling mission, call the `DjiInterestPoint_Init` API to initialize the POI module.
...
### 2. Set the Circling Speed

Before starting the POI mission or while the mission is in progress, call the `DjiInterestPoint_SetSpeed` API to configure the circling speed.

```c
    DjiInterestPoint_SetSpeed(5.0f);
```

### 3. Select the Point of Interest and Start Circling

Select an appropriate point of interest and call the `DjiInterestPoint_Start` API to start the circling mission.  
The circling radius is automatically calculated based on the aircraft’s current position and the GPS coordinates of the target point.
...
### 4. Monitor the Circling Status

Before starting the POI mission, it is recommended to register a POI mission status callback.  
During mission execution, the registered callback function can be used to receive real-time mission status information (such as mission state, circling speed, and circling radius), helping to ensure flight safety.
...
### 5. Stop the Circling Mission

While the POI mission is running, call the `DjiInterestPoint_Stop` API to stop the circling mission.
