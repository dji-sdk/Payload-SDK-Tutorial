...
An application with the function of <a href="../basic-function/info-management.html" target="_blank" rel="noopener noreferrer">Data Subscription</a> can facilitate users to obtain information about the basic parameters of the aircraft:
...
:::tip Note:
 When the modulo of the position command is not 0, the aircraft will fly forward at the specified speed, otherwise, the aircraft will hover at the specified position.
:::
...
Location-related data obtained through the PSDK can be referenced in <a href="../basic-function/info-management.html#subscription-topics" target="_blank" rel="noopener noreferrer">Data Subscription</a>.
...
During the flight mission, real-time monitoring of the aircraft's status and alert information is necessary to assist remote pilots or program decisions. Refer to the <a href="../basic-function/info-management.html" target="_blank" rel="noopener noreferrer">Data Subscription</a> and <a href="../advanced-function/hms-management.html" target="_blank" rel="noopener noreferrer">HMS Features</a> chapter for further details.
...
```
:::tip Note:
 If you do not change the settings in the Joystick during the flight of the aircraft, you can only set the Joystick mode when the aircraft starts to fly.
:::

2. Set up Joystick control commands and perform Joystick functions
:::tip Note:
 The z-axis position in `joystickCommand` is the absolute height relative to the height of the take-off point.
:::
...
```
### 7. Battery Capacity Go-Home Callback Interfaces

#### Register Battery Capacity Go-Home Callback
- Function: Registers a callback function for battery capacity related to Go-Home. Through this callback, information such as the aircraft's remaining flight time and the battery threshold for low-battery automatic return-to-home can be obtained.
- Supported Products: Matrice 400, Matrice 4T/4E
- For usage instructions, refer to the calling example `DjiTest_FlightControlGetBatteryCapacityGoHome` in `samples\sample_c\module_sample\flight_control\test_flight_control.c`.

```c
T_DjiReturnCode DjiFlightController_RegisterBatteryCapacityGohomeCallBack(FcCmderBatteryCapacityGohomeCbFunc callback);
```

#### Unregister Battery Capacity Go-Home Callback
- Function: Unregisters the battery capacity Go-Home callback function. This function can be called when it is no longer necessary to obtain information such as the aircraft's remaining flight time or the low-battery return-to-home threshold.
- Supported Products: Matrice 400, Matrice 4T/4E
- For usage instructions, refer to the calling example `DjiTest_FlightControlGetBatteryCapacityGoHome` in `samples\sample_c\module_sample\flight_control\test_flight_control.c`.
...
### 8. PSDK Control Interfaces in Attitude (ATTI) Mode

#### Set PSDK Control State in ATTI Mode
- Function: Enables or disables PSDK control of the aircraft when the aircraft is in Attitude (ATTI) mode.
- Supported Products: Matrice 4T/4E
- Set `true` to allow control in ATTI mode, and set `false` to disable control.

```c
T_DjiReturnCode DjiFlightController_SetControlInAttitudeModeEnabled(bool enabled);
```
#### Get PSDK Control Status in ATTI Mode
- Function: Gets the PSDK control status when the aircraft is in Attitude (ATTI) mode.
- Supported Products: Matrice 4T / 4E
```c
...
```
:::tip Note:
The Flycart 100 features an obstacle avoidance system that is distinct from other models.
:::
