...
### List of Known Issues
<table width="85%" style="hyphens: auto; display: table; table-layout:fixed;"> <thead> <tr> <th width="15%">Product</th> <th>Problem Description</th> <th>Avoidance Measures</th> </tr> </thead> <tbody> <tr> <td>M400</td> <td>When upgrading the EPort V2 development kit MCU firmware using DA2, there is a probability that the process will stop after reaching 100% and cannot continue.</td> <td>Manually power off and then power on the PSDK device to complete the upgrade successfully.</td> </tr> <tr> <td>M400</td> <td>Positioning function is currently not supported.</td> <td>Will be supported in future versions.</td> </tr> <tr> <td>M400</td> <td>Small data transmission between payloads is not yet supported.</td> <td>Will be supported in future versions.</td> </tr><tr>
    <td>M400</td>
    <td>When connecting a payload to the E-Port V2 development board via Skyport V3, there is a probability of unstable connection.</td>
    <td>This issue does not occur if you use a coaxial cable from E-Port V2 to connect the E-Port V2 development board and then the payload, or if you connect the payload device directly to SkyPort V3.</td>
</tr> </tbody> </table>
...
* Supports DJI SDK Certified Chip (DJI SDK CC) (The description and usage instructions for the DJI SDK CC can be found by [clicking here](https://developer.dji.com/doc/payload-sdk-tutorial/en/payload-quick-start/quick-guide/sdk-cc.html))
...
For details: [Meida File Metadata](https://developer.dji.com/doc/payload-sdk-tutorial/en/advanced-function/media-file-metadata.html).
